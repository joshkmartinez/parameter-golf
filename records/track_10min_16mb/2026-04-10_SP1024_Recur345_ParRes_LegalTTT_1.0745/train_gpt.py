...
class Hyperparameters():
    ...
    # Pre-quant AdamW TTT (runs after EMA, before GPTQ)
    ...
    # Legal Score-First TTT (PR #461/#549 pattern - chunk-based, score-before-update)
...
class GPT(nn.Module):
    ...
    def _get_virtual_layers(self) -> list[int]:
        ...
        e.g. with num_layers=11 and recur_layers=[3,4,5]:
            [0,1,2, 3,4,5, 3,4,5, 6,7,8,9,10]
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        # Use virtual layers for depth recurrence
        ...
        for vi in range(num_dec):
            ...
            if is_parallel_mode:
                ...
                attn_out = block.attn(block.attn_norm(attn_in) * block.ln_scale_factor)
...
def eval_val_sliding_etlb(h, device, val_data, base_model, batch_seqs=32):
    base_model.eval()
    logits_fn = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True)
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        for _ in range(h.etlb_steps):
            biased_ctx = logits_f[:, :context_size, :] + cur_bias[None, None, :]
            probs = F.softmax(biased_ctx, dim=-1)
            targets_ctx = y_batch[:, :context_size].reshape(-1)
            probs_flat = probs.reshape(-1, h.vocab_size)
            one_hot = torch.zeros_like(probs_flat)
            one_hot.scatter_(1, targets_ctx.unsqueeze(1), 1.0)
            grad = (probs_flat - one_hot).mean(dim=0)
            cur_bias = (cur_bias - h.etlb_lr * grad).clamp(-h.etlb_clip, h.etlb_clip)
        bias = cur_bias.detach()
        biased_logits = logits_f + bias[None, None, :]
        nll = F.cross_entropy(biased_logits.reshape(-1, biased_logits.size(-1)),
...
def train_model(h: Hyperparameters, device: torch.device, val_data: ValidationData):
    ...
    while True:
        ...
        # Activate depth recurrence at recur_start_step
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    # Pre-quant AdamW TTT (runs after EMA, before GPTQ quantization)
    if h.prequant_ttt_enabled:
        # Activate recurrence for TTT if not already active
        if not base_model._recurrence_active:
            base_model.set_recurrence_active(True)
            log(f"recurrence:activated for TTT, virtual_layers={base_model._get_virtual_layers()}")
        log(f"prequant_ttt:starting (epochs={h.prequant_ttt_epochs}, lr={h.prequant_ttt_lr}, freeze={h.prequant_ttt_freeze_blocks})")
        prequant_ttt_adapt_adamw(h, base_model, device, val_data.val_tokens, rank=h.local_rank if h.distributed else 0, world_size=h.world_size if h.distributed else 1)
        # Re-compile after TTT since weights changed
        torch._dynamo.reset()
        compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
        timed_eval("post-ttt pre-quant", eval_val, h, device, val_data, compiled_model)
    ...
    # Activate recurrence for eval if it was active during training
    if base_model._recurrence_active:
        eval_model.set_recurrence_active(True)
        log(f"eval_model:recurrence activated, virtual_layers={eval_model._get_virtual_layers()}")
    ...
    if h.ttt_enabled and h.sliding_window_enabled:
        ...
        timed_eval("quantized_sliding_ttt", eval_val_sliding_ttt, h, eval_model, h.rank, h.world_size, device, val_data.val_tokens, val_data.base_bytes_lut, val_data.has_leading_space_lut, val_data.is_boundary_token_lut, h.eval_stride, 32)
